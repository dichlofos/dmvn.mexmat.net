<?php
  ...
  for ($i = 0; $i < strlen($strStrToUpperAH); $i++) $arrStrToUpperAH[] = $strStrToUpperAH[$i];
  for ($i = 0; $i < strlen($strStrToUpperAL); $i++) $arrStrToUpperAL[] = $strStrToUpperAL[$i];
  ...
  for ($i = 0; $i < strlen($strStrToLowerAH); $i++) $arrStrToLowerAH[] = $strStrToLowerAH[$i];
  for ($i = 0; $i < strlen($strStrToLowerAL); $i++) $arrStrToLowerAL[] = $strStrToLowerAL[$i];
  ...
  function ArrayToString($arrData, $strSep)
  {
    ...
    $strResult = $arrData[0];
    for ($i = 1; $i < count($arrData); $i++) $strResult .= $strSep.$arrData[$i];
    return $strResult;
  }
  // -------------------------------------------------------------
  ...
  {
    if (!isset($arrTest)) return true;
    if (!is_array($arrTest)) return true;
    if (!count($arrTest)) return true;
    return false;
  }
  // -------------------------------------------------------------
  ...
  {
    if (!isset($strTest)) return true;
    if (!$strTest) return true;
    return false;
  }
  // -------------------------------------------------------------
  ...
  {
    global $$strVarName;
    if (!isset($$strVarName)) return $strDefaultValue;
    else
      if (is_string($$strVarName)) return (get_magic_quotes_gpc() ? stripslashes($$strVarName) : $$strVarName);
      else return $strDefaultValue;
  }
  // -------------------------------------------------------------
  ...
  function FilterLowASCII($strString)
  {
    for ($i = 0; $i < strlen($strString); $i++)
      if (ord($strString[$i]) < 32) $strString[$i] = ' ';
    return $strString;
  } 
  // -------------------------------------------------------------
  ...
  {
    while (strlen($strNum) < $nWidth) $strNum = '0'.$strNum;
    return $strNum;
  }
?>
